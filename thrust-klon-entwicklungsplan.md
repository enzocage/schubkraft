...
| M2 | Schiffsphysik | Thrust, Gravitation, Trägheit, Fuel-Verbrauch | Flugverhalten „fühlt sich nach Thrust an" (Tuning-Session) |
...
DRAG         = 0      (Thrust hat KEINEN Luftwiderstand — Trägheit ist das Spiel)
FUEL_BURN    = 1 / Frame Thrust, Schießen kostet nichts, Traktorstrahl 1/Frame
```
...
Klassisches Thrust nutzt de facto einen **starren Stab**, kein Seil:
...
- Gesamtschwerpunkt: Thrust wirkt nur aufs Schiff → Drehmoment ums System entsteht automatisch → das berühmte Aufschaukeln/Pendeln.
...
→ Performt problemlos, wenn das Hatching **einmal pro Level in ein Offscreen-Canvas gebacken** wird (Terrain ist statisch); pro Frame nur `drawImage` mit Kamera-Offset.

### 3.3 Objekte (alles Linien, keine Fills)
- **Schiff:** Pfeil-Dreieck mit eingezogener Kerbe (wie Screenshot 2/3), Thrust-Flamme = 2–3 flackernde Liniensegmente (random Länge/Frame).
- **Pod:** Kreis auf Sockel-Dreieck (Screenshot 1/3 unten rechts).
- **Turret:** Halbmond/Kuppel-Bogen, Rohr rotiert Richtung Spieler.
...
|---|---|
| **Thrust** | Weißes Rauschen (Loop-Buffer) → Bandpass 300–900 Hz, Cutoff moduliert mit LFO 8 Hz + Zufall; Gain mit 30 ms Attack/Release an Taste gekoppelt. Dazu leiser Puls-Oszillator ~55 Hz für Körper. |
...
**Implementierung:** Eine `sfx(name, params)`-Factory, die pro Aufruf einen kurzlebigen Node-Graph erzeugt (Oszillator/Noise → Filter → Gain → Master) und sich selbst per `onended` aufräumt. Dauerklänge (Thrust, Strahl, Alarm) als persistente Graphen mit Gain-Steuerung. AudioContext erst nach erster User-Geste resumen (iOS-Pflicht).
...
│  Rotation:           │  unten: THRUST       │
│  horizontales        │  (Finger liegt auf = │
│  Drag relativ zum    │   Schub an)          │
...
**Detail-Regeln**
- **Rotation links:** Beim `pointerdown` Ankerpunkt merken. `dx` zum Anker steuert Drehrichtung; Totzone ±8 px; jenseits ±8 px volle (digitale) Rotation — Thrust ist digital, also kein Analog-Mapping nötig. Finger nachziehen rebasiert den Anker langsam (Drift-Korrektur), damit man nie am Displayrand „ansteht".
- **Thrust rechts unten:** Halten = Schub. Kein Tap-Toggle — die direkte Kopplung Finger↔Triebwerk ist essenziell fürs Pendel-Feintuning.
...
- **Schild/Strahl:** Zweiter Finger in der rechten Hälfte während Thrust-Finger liegt → Schild. Alternativ konfigurierbar: Long-Press links.
...
**Desktop parallel:** ←/→ rotieren, ↑ oder A = Thrust, Space = Feuer, Shift = Schild, P = Pause, E = Editor.
...
6. Nach Level 6: Zyklus wiederholt mit **invertierter Gravitation** und unsichtbarem Terrain-Blinken (klassische Thrust-Steigerung) — beides nur Flags im Theme.

---

## 8. Risiken & Tuning-Notizen

- **Pendel-Stabilität:** Substeps (4× pro Frame) für den Constraint, sonst Energie-Aufschaukeln. Früh in M5 mit Worst-Case testen (Vollthrust quer zur Stabachse).
...
- **iOS-Audio:** Context-Resume an ersten Touch binden; Thrust-Loop-Gain nie auf exakt 0 (sondern 0.0001), verhindert Click-Artefakte.
...
1. M1+M2 in einem Rutsch (Boilerplate ist bei dir Routine).
